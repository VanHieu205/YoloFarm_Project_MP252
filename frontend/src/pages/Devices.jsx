...
import { getDevices, updateDeviceStatus } from '../api/deviceApi'
...
const Devices = () => {
  ...
  const fetchDevices = async () => {
    try {
      ...
      // response.data nếu backend trả axios response
  ...
  const TypeSection = ({ typeKey }) => {
    ...
    const percentage =
      typeDevices.length > 0
        ? Math.round((activeCount / typeDevices.length) * 100)
        : 0
  ...
  const totalActive = devices.filter(
    (d) => d.status === 'on' || d.status === 'active'
  ).length
    ...
    >
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: '#6b7280' }}>
          Theo dõi và điều khiển trạng thái thiết bị
        </p>
      </div>
