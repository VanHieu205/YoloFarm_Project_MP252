...
const SensorChart = () => {
  ...
  // Load biểu đồ khi đổi mode / date
  useEffect(() => {
    fetchChart(mode, date)
  }, [mode, date, fetchChart])

  // Realtime latest — luôn chạy
  useEffect(() => {
    fetchLatest()
  ...
  // ── Current Stats (dùng latestStats) ─────────────────────────────────────
  const currentStats = [
    ...
    { label: "Độ ẩm không khí", value: latestStats.humidity      != null ? `${latestStats.humidity}%`        : "—", unit: "Phần trăm", icon: Droplet,     color: "#3b82f6", bgColor: "rgba(59,130,246,0.1)"  },
    ...
    { label: "Độ ẩm đất",       value: latestStats.soil_moisture  != null ? `${latestStats.soil_moisture}%`  : "—", unit: "Phần trăm", icon: Sprout,      color: "#22c55e", bgColor: "rgba(34,197,94,0.1)"   },
    { label: "CO₂",             value: latestStats.co2            != null ? String(latestStats.co2)           : "—", unit: "ppm",       icon: Wind,        color: "#8b5cf6", bgColor: "rgba(139,92,246,0.1)"  },
  ]
  ...
  return (
    <div className="card">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "var(--spacing-lg)", flexWrap: "wrap" }}>
        ...
        {/* Date picker — chỉ hiện khi mode=day */}
        {mode === "day" && (
          <input
            type="date"
            value={date}
